...
const demoOperations=[
 ...omniformCatalog.operations.map(item=>({...structuredClone(item),inputSchema:item.input,outputSchema:item.output,available:item.id!=='apply_plan',unavailableReason:item.id==='apply_plan'?'demo runtime is read-only':null,implementedBy:item.id==='apply_plan'?null:'demo-transport',runtimeOperation:{get_capability:'getCapability',generate_plan:'generatePlan',apply_plan:'applyPlan'}[item.id]})),
