...
const demoOperations=[
 ...omniformCatalog.operations.map(item=>{const mutation=item.mutation;return {...structuredClone(item),inputSchema:item.input,outputSchema:item.output,available:!mutation,unavailableReason:mutation?'demo runtime is read-only':null,implementedBy:mutation?null:'demo-transport',runtimeOperation:{get_capability:'getCapability',generate_plan:'generatePlan',apply_plan:'applyPlan',resolve_capability:'resolveCapability',get_capability_realisation:'getCapabilityRealisation',list_attention:'listAttention',accept_capability_gap:'acceptCapabilityGap'}[item.id]}}),
